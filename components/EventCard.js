...
function EventCard(props) {
    return <div
        ...
        <div className="h-52 rounded-lg bg-center bg-cover"
            ...
            <h3 className={"bg-[#101014]/70 font-droid shadow block absolute rounded py-1 px-2 m-2 text-xs"}>{props.event.author}</h3>
            <h3 className={"bg-[#101014]/70 font-droid shadow block absolute rounded py-1 px-2 m-2 text-xs top-0 right-0"}>{dayjs.unix(Math.floor(Date.parse(props.event.created_at) / 1000)).fromNow()}</h3>
        </div>
        <div className={" p-2"}>
            <Text noOfLines={2}
            ...
            <div className={"text-xs pt-1 font-droid"}>
                {
                    props.event.tags.split(",").map(
                        tag => <Tag m={.5} size={"sm"} colorScheme={"linkedin"} key={tag}>
                            {tag}
                        </Tag>
                    )
                }
