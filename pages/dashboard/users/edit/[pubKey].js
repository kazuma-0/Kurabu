...
import signUpFormReducer, {formInitialState} from "../../../../reducers/signUpFormReducer";
import {
    AlertDialog,
    AlertDialogBody,
...
function EditUser() {
    ...
    async function Update() {
        setLoading(true);
        try {
            const {data} = await frontendClient.post("/updateUser", {
                ...user,
                ...formState
            }, {
                headers: {
    ...
    async function deleteUser() {
        try {
            await frontendClient.post('/deleteUser', {
                id: user.id
            },{
                headers: {
                    auth: publicKey
    ...
    return <div className={"h-cover"}>
        <UserForm roles={roles} onSubmit={handleSubmit} onChange={dispatcher} formState={formState} onClick={null}
                  loading={loading} onClick1={Update} deleteUser={onOpen}/>
        <AlertDialog leastDestructiveRef={leastRef} isOpen={isOpen} onClose={onClose}>
            <AlertDialogOverlay>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        Are you sure you want to remove <span className={"underline text-red-300"}>{user?.name}</span> from the club?
                    </AlertDialogHeader>
                    <AlertDialogFooter>
