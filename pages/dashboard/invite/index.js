...
function Invite() {
    ...
    return <div className={"h-cover"}>
        <Stack direction={"row"}>
            <Button onClick={onOpen} colorScheme={"linkedin"}>New code</Button>
            <Button onClick={listView.onOpen} colorScheme={"linkedin"}>List view</Button>
        </Stack>
            ...
            <ModalOverlay>
                <ModalContent>
                    ...
                    <ModalBody>
                        ...
                        }} placeHolder={"How many invite codes do you want?"} value={count}/>
                    </ModalBody>
                    <ModalFooter>
                        <Button onClick={generateInviteCodes} variant={"outline"}
                                colorScheme={"whatsapp"}>Generate</Button>
                    </ModalFooter>
                    <ModalCloseButton/>
                </ModalContent>
            </ModalOverlay>
        ...
        <Modal isOpen={listView.isOpen} onClose={listView.onClose}>
            <ModalOverlay>
                <ModalContent>
                    ...
                    <ModalCloseButton/>
                    <ModalBody>
                        {codes?.map(code => <div key={code.id}>{code.code}</div>)}
                    </ModalBody>
                </ModalContent>
            </ModalOverlay>
        </Modal>
        <div className="grid grid-cols-4 gap-10 py-10">
            {codes?.map(code => <div key={code.id} onClick={() => {
                copy(code.code)
            }}
